...
// HTML-aware variant for already-rendered content (work cache / rebuilt output):
// converts digits in text nodes only, leaving <…> tags — and the digits inside
// their attributes (id="x2", data-t="1") — untouched.
function toPersianDigitsHtml(html) {
  return String(html).replace(/<[^>]*>|[0-9]/g, (m) =>
    m[0] === "<" ? m : PERSIAN[Number(m)],
  );
}

module.exports = { toPersianDigits, toPersianDigitsHtml };
