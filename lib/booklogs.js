...
class BookLogs {
  ...
  list() {
    ...
    for (const slug of fs.readdirSync(this.baseDir)) {
      ...
      // Real card count = request records; `entries` counts every tape line
      // (requests + responses + markers ≈ 2× the cards), so expose both.
      let cards = 0;
      for (const e of entries) if (e.t === 'request') cards += 1;
      let lastTs = null;
      let percent = null;
      let doneWords = null;
      ...
      for (let i = entries.length - 1; i >= 0; i--) {
        ...
        if (entries[i].t === 'progress' && entries[i].data && entries[i].data.percent != null && percent == null) {
          ...
          doneWords = entries[i].data.doneWords ?? null;
          targetWords = entries[i].data.targetWords ?? null;
        }
      }
      let size = 0;
