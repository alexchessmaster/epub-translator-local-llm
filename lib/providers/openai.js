...
function makeOpenAiProvider({ baseUrl, apiKey }) {
  ...
  // `onThinking` is accepted for interface parity with the Ollama adapter and
  // never called: OpenAI-compatible endpoints expose no reasoning stream.
  async function chatStream({ model, messages, think, numCtx, temperature = 0.3, signal, onDelta, onThinking, format = 'json' }) {
