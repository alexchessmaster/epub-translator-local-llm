// lib/ollama.js — talk to the local Ollama server.
// chatStream streams NDJSON tokens and calls onDelta per content chunk, which is
// what lets the dashboard show the Persian text typing in live.
...
async function chatStream({ model, messages, think, numCtx = 8192, temperature = 0.3, signal, onDelta, format = 'json' }) {
  const payload = {
    model,
    messages,
    stream: true,
  ...
  if (!res.ok) {
    ...
    throw new Error(`Ollama /api/chat failed: HTTP ${res.status} ${body.slice(0, 200)}`);
  }
...
module.exports = { listModels, chatStream, chatJson };
