...
// Ratio of Latin letters among script letters — used to detect paragraphs that
// are still English (Persian text is non-Latin).
function latinRatio(text) {
  const letters = (text.match(/[A-Za-z]/g) || []).length;
  const script = (text.match(/[A-Za-z؀-ۿ]/g) || []).length;
  return script ? letters / script : 0;
}
...
function isTranslatable(flat) {
  ...
  if (wordCount(s) < 2 && !/[A-Za-z؀-ۿ]/.test(s)) return false;
  return true;
}
...
function rtlTag(tag, attrs) {
  attrs = attrs.replace(/\s*(?:xml:)?lang\s*=\s*"[^"]*"/g, '');
  attrs = attrs.replace(/\s*dir\s*=\s*"[^"]*"/g, '');
  attrs = attrs.replace(/\s+$/, '');
  return `<${tag} dir="rtl" lang="fa" xml:lang="fa"${attrs}>`;
}

function setRtl(xhtml) {
  let out = xhtml.replace(/<html\b([^>]*)>/, (g, attrs) => rtlTag('html', attrs));
  out = out.replace(/<body\b([^>]*)>/, (g, attrs) => rtlTag('body', attrs));
  return out;
}

function updateOpfMetadata(buffer) {
  return Buffer.from(
    buffer
      .toString('utf8')
      .replace(/<dc:language[^>]*>[^<]*<\/dc:language>/, '<dc:language>fa</dc:language>')
  );
}
...
function rebuildEpub({ entries, buffers, changed, opfPath, outPath }) {
  return new Promise((resolve, reject) => {
    ...
    for (const e of entries) {
      ...
      if (data === undefined) {
        ...
        if (e.name === opfPath) data = updateOpfMetadata(data);
      }
...
module.exports = {
  ...
  placeholderTokens, UnitExtractor, setRtl, updateOpfMetadata,
  readEpub, findOpfPath, listTextFiles, rebuildEpub, buildXhtml,
};
