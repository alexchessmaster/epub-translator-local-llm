...
class TranslatorJob {
  ...
  buildGlossaryLine() {
    const users = (this.glossaryStore ? this.glossaryStore.list() : []).filter(
      (e) => e.source === "user",
    );
    const autos = (
      this.glossaryAutoStore ? this.glossaryAutoStore.list() : []
    ).sort((a, b) => (b.freq || 0) - (a.freq || 0));
    const limit = this.prompt.glossaryLimit || 20;
    const chosen = [
      ...users,
      ...autos.slice(0, Math.max(0, limit - users.length)),
    ];
  ...
  async translateSingle(file, unit, slotContent, translatedSlots) {
    ...
    const messages = [
      ...
      {
        role: "user",
        content: buildSinglePrompt(this.prompt, flat, this.glossaryLine, langs),
      },
    ];
    this.emit("request", {
      id: rid,
    ...
    // Deterministic post-processing: Persian digits in code, not by the model.
    const out = toPersianDigits(text);
    this.logEntry({
      id: rid,
      phase: "response",
      ts: this.time(),
    ...
    this.emit("response", {
      id: rid,
      durationMs: durMs,
      pairs: [{ n: 1, src: flat, tgt: out }],
    });
    this.newWords += epub.wordCount(flat);
    this.doneWords += epub.wordCount(flat);
    slotContent.set(unit.slot, epub.restorePlaceholders(out, unit.mapping));
