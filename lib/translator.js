...
function cacheKey(book, model, targetLang, file) {
  ...
  parts.push(model.replace(/[^A-Za-z0-9]/g, '_'), targetLang || 'fa', path.basename(file));
...
class TranslatorJob {
  ...
  buildGlossaryLine() {
    const users = (this.glossaryStore ? this.glossaryStore.list() : []).filter((e) => e.source === 'user');
    const autos = (this.glossaryAutoStore ? this.glossaryAutoStore.list() : [])
  ...
  async run() {
    ...
    for (let i = 0; i < this.textFiles.length; i++) {
      ...
      const readPath = findCachePath(this.bookName, this.model, this.targetLang, f, this.workDir);
      if (readPath) {
        const cres = new epub.UnitExtractor().walk(fs.readFileSync(readPath, 'utf8'));
        for (const cu of cres.units) {
          const opener = cres.out[cu.slot - 1] || '';
          if (opener.includes('data-t="1"')) this.doneWords += epub.wordCount(cu.flat);
        }
      }
      this.emit('progress', {
        phase: 'scan',
    ...
    this.emit('progress', {
      phase: 'translate',
      percent: this.targetWords ? Math.min(100, (100 * this.doneWords) / this.targetWords) : 0,
      doneWords: this.doneWords,
      targetWords: this.targetWords,
      wordsPerMin: 0,
      etaSec: null,
    ...
    if (this.prompt.glossaryEnabled !== false && this.glossaryStore) {
      ...
      const missing = candidates
        .filter((n) => !this.glossaryStore.get(n.name) && !autoStore.get(n.name))
      ...
      if (missing.length) {
        try {
          ...
          const userCount = (this.glossaryStore.list().filter((e) => e.source === 'user')).length;
          this.emit('glossary', { added, count: userCount + autoStore.list().length });
        } catch (e) {
    ...
    this.emit('progress', {
      phase: 'rebuild',
      percent: this.targetWords ? Math.min(100, (100 * this.doneWords) / this.targetWords) : 100,
      doneWords: this.doneWords,
      targetWords: this.targetWords,
      wordsPerMin: 0,
      etaSec: null,
    ...
    return { outPath, elapsedSec, wordsPerMin: wpm, doneWords: this.doneWords, targetWords: this.targetWords };
  }
  ...
  queueFile(file, bookPos, fw) {
    ...
    const cachePath = findCachePath(this.bookName, this.model, this.targetLang, file, this.workDir);
    if (cachePath) {
    ...
    for (const u of units) {
      ...
      this.emit('queue', { id: rid, model: this.model, book: this.bookName, file, paragraphs: [u.flat], ts, targetLang: this.targetLang });
    }
  }
  ...
  async translateFile(file, bookPos, fw, changed) {
    ...
    const readPath = findCachePath(this.bookName, this.model, this.targetLang, file, this.workDir);
    const writePath = path.join(this.workDir, cacheKey(this.bookName, this.model, this.targetLang, file));
    ...
    for (let attempt = 0; attempt < 3 && pending.length; attempt++) {
      ...
      try {
        const r = await this.provider.chatStream({
          ...
          onThinking: attempt === 0 ? (d) => this.emit('think', { id: rid, delta: d }) : undefined,
        });
        text = r.text;
      } catch (e) {
...
module.exports = { runTranslation, TranslatorJob, cacheKey, findCachePath, buildUserPrompt, buildSinglePrompt, AbortError, extractNames, buildGlossary };
