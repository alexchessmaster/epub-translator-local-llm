...
class GlossaryStore {
  ...
  // Re-read the on-disk entries into memory. Reads are disk-backed so that any
  // instance — a running job's store, a fresh store created by an API route —
  // always sees the latest edits (the user can fix the glossary mid-run and the
  // very next prompt picks it up).
  _read() {
