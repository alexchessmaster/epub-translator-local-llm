...
const { buildSinglePrompt, cacheKey } = require('./translator');
...
async function fixParagraph({
  bookData, file, src, model, cacheModel, sourceLang, targetLang,
  ...
  const cachePath = path.join(workDir, cacheKey(cacheModel, targetLang, file));
  ...
  fs.writeFileSync(cachePath, epub.setLang(epub.buildXhtmlMarked(parsed.out, slotContent, translatedSlots), targetLang, dir));
...
async function rebuildOutputs({ bookData, bookName, model, targetLang, workDir, outDir }) {
  ...
  for (const f of textFiles) {
    const cachePath = path.join(workDir, cacheKey(model, targetLang, f));
    if (fs.existsSync(cachePath)) {
