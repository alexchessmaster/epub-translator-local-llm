...
const { toPersianDigits, toPersianDigitsHtml } = require('./digits');
...
async function fixParagraph({
  ...
  if (manualTgt != null) {
    ...
    // Consistent digits for automated re-translates (manual edits are kept verbatim).
    text = toPersianDigits((r.text || '').trim());
...
async function rebuildOutputs({ bookData, bookName, model, targetLang, workDir, outDir }) {
  ...
  for (const f of textFiles) {
    ...
    if (cachePath) {
      // Normalize digits in already-translated slots (ASCII → Persian) so rebuilt
      // outputs are consistent even for content cached before the digit algorithm.
      const { parsed, slotContent, translatedSlots } = loadCache(cachePath);
      for (const s of translatedSlots) slotContent.set(s, toPersianDigitsHtml(slotContent.get(s)));
      changed.set(f, Buffer.from(epub.stripMarkers(epub.buildXhtmlMarked(parsed.out, slotContent, translatedSlots))));
    }
  }
